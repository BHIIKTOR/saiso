...
export const localStrategyTestHarnessAction: Action = {
  ...
  similes: ['LOCAL_STRATEGY_TEST_HARNESS', 'LOCAL_STRATEGY_TEST_HARNESS_ADAPTER_EVM'],
  description: 'Run deterministic local strategy scenarios (EVM-specific adapter)',
  validate: async () => true,
  handler: async (
    ...
    const response = {
      ...
      meta: {
        requestId: 'saiso-evm-' + Date.now().toString(36),
      },
    };
