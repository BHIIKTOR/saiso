...
describe('quote_and_swap action', () => {
  afterEach(() => {
    globalThis.fetch = originalFetch;
  });

  it('returns a normalized EVM quote response', async () => {
    ...
    const runtime = {
      getSetting: mock((key: string) => key === 'ZEROX_SWAP_API_BASE' ? 'https://zero.example/swap/v1' : undefined),
    } as any;

    const result = await quoteAndSwapAction.handler(
      runtime,
      { content: { chainFamily: 'evm', sellToken: 'ETH', buyToken: 'USDC', sellAmount: '1000000000000000000' } } as any,
      {} as any,
      {} as any,
      undefined as any
    );
    ...
    expect(result.data.provider).toBe('0x');
  ...
  it('fails fast for hosted 0x when ZEROX_API_KEY is missing', async () => {
    ...
    const result = await quoteAndSwapAction.handler(
      runtime,
      { content: { chainFamily: 'evm', sellToken: 'ETH', buyToken: 'USDC', sellAmount: '1' } } as any,
      {} as any,
      {} as any,
      undefined as any
    );
  ...
  it('allows custom quote providers without ZEROX_API_KEY', async () => {
    ...
    const result = await quoteAndSwapAction.handler(
      runtime,
      { content: { chainFamily: 'evm', quoteProvider: 'custom', quoteUrl: 'https://quotes.example/swap', sellToken: 'ETH', buyToken: 'USDC', sellAmount: '1' } } as any,
      {} as any,
      {} as any,
      undefined as any
    );
  ...
  it('returns normalized provider failures', async () => {
    globalThis.fetch = mock(async () => ({
      ok: false,
      status: 502,
