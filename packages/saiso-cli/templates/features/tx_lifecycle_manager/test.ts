...
describe('tx_lifecycle_manager action', () => {
  afterEach(() => {
    globalThis.fetch = originalFetch;
  });

  it('tracks an EVM transaction lifecycle', async () => {
    ...
    const result = await txLifecycleManagerAction.handler(
      runtime,
      { content: { chainFamily: 'evm', txHash, requiredConfirmations: 2 } } as any,
      {} as any,
      {} as any,
      undefined as any
    );
    ...
    expect(result.operation).toBe('tx_lifecycle_manager');
