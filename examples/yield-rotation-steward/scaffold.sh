#!/usr/bin/env bash
set -euo pipefail
PROJECT_NAME="${1:-yield-rotation-steward}"
EXAMPLE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

saiso new "${PROJECT_NAME}" --env testnet --agent-name "YieldRotationSteward" --description "Yield rotation bot with PnL, limits, and Privy wallet controls" --yes
cd "${PROJECT_NAME}"
bun install
...
cp "${EXAMPLE_DIR}/config/yield-rotation-limits.example.json" config/yield-rotation-limits.json
cp "${EXAMPLE_DIR}/src/yield-rotation.example.ts" src/yield-rotation.example.ts
